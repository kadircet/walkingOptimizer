...
void pushLoc(float x, float y, float z)
{
	dy_data_set_float(dy_data_retrieve("robot.position.x"), x);
	dy_data_set_float(dy_data_retrieve("robot.position.y"), y);
	dy_data_set_float(dy_data_retrieve("robot.position.z"), z);
}

void pushRot(float roll, float pitch, float yaw)
{
	dy_data_set_float(dy_data_retrieve("robot.angle.x"), roll);
	dy_data_set_float(dy_data_retrieve("robot.angle.y"), pitch);
...
void sendFrameData(const MocapFrame &frame)
{
	...
	if(bodies.size()==1)
	{
		pushLoc(bodies[0].location().x, bodies[0].location().y, bodies[0].location().z);
		Quaternion4f ori = bodies[0].orientation();
		float roll  = atan2(2*ori.qy*ori.qw - 2*ori.qx*ori.qz, 1 - 2*ori.qy*ori.qy - 2*ori.qz*ori.qz);
